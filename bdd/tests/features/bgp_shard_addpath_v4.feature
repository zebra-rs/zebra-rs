@serial
@bgp_shard_addpath_v4
Feature: BGP IPv4-unicast AddPath session-up sync at N>1 (all paths sync; per-path withdraw + peer-down)
  ...
  Scenario: the late AddPath peer z4 gets BOTH paths on sync
    ...
    # z4 starts AFTER both candidates exist on z3, so it learns them only via
    # z3's `route_sync_ipv4` AddPath dump (every candidate, each path-id'd).
  ...
  Scenario: z1 withdraws; only z1's path-id is withdrawn from the synced z4
    Given the test topology exists
    # z1 stops originating. z4 — which learned z1's path via the AddPath sync
