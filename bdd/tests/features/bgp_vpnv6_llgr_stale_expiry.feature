@serial
@bgp_vpnv6_llgr_stale_expiry
Feature: LLGR stale VPNv6 rows expire after the stale time when the peer never returns
  ...
  Scenario: Setup topology and establish the session
    ...
    And I execute "ip -6 addr add 2001:db8::1/64 dev vz1ns" in namespace "z1"
    And I execute "ip -6 addr add 2001:db8::2/64 dev vz2ns" in namespace "z2"
  ...
  Scenario: z2 holds z1's route
    ...
    Then show command "show bgp vpnv6" in namespace "z2" should eventually contain "2001:db8:1::/64"
  ...
  Scenario: z1 dies for good and its stale route is swept after the stale time
    ...
    Then show command "show bgp vpnv6" in namespace "z2" should contain "2001:db8:1::/64"
    # ... and gone once the 10-second stale time has elapsed. Pre-fix the
    # row stayed for the daemon's life.
    And show command "show bgp vpnv6" in namespace "z2" should eventually not contain "2001:db8:1::/64"
