@serial
@bgp_evpn_addpath_flip
Feature: EVPN AddPath members receive every candidate, and a superseded path-id is withdrawn on a flip (IPv4 Type-5)
  ...
  │ Type-5 10.1.0.0/24│          │ z1, z4, z3  │  send    │ receive     │
  └─────────────┘          └─────────────┘          └─────────────┘
  ...
  z1 and z4 both originate an EVPN Type-5 for 10.1.0.0/24 under the SAME route
