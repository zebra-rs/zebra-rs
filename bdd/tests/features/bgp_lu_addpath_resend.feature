@serial
@bgp_lu_addpath_resend
Feature: AddPath labeled-unicast peers must not re-send unchanged candidates to each other (IPv4)
  ...
  └────┬─────────────────────┬────┘
   ...
   └───────┘   directions └───────┘
  ```
  Session addresses are 192.168.90.N; router-ids 10.90.0.N. Both routers
  originate 10.90.0.0/24 in labeled-unicast.

  Scenario: Setup topology; both routers hold both candidates
    Given a clean test environment
    When I create bridge "br0"
    And I create namespace "z1" with IP "192.168.90.1/24" on bridge "br0"
    And I create namespace "z2" with IP "192.168.90.2/24" on bridge "br0"
    And I start zebra-rs in namespace "z1"
    And I start zebra-rs in namespace "z2"
    And I apply config "z1.yaml" to namespace "z1"
    And I apply config "z2.yaml" to namespace "z2"
    Then BGP session in "z1" to "192.168.90.2" should eventually be "Established"
    And BGP session in "z2" to "192.168.90.1" should eventually be "Established"
    And show command "show bgp labeled-unicast" in namespace "z1" should eventually contain "10.90.0.0/24"
    And show command "show bgp labeled-unicast" in namespace "z2" should eventually contain "10.90.0.0/24"
  ...
  Scenario: The sessions settle instead of re-sending the unchanged candidates forever
    ...
    Then BGP neighbor "192.168.90.2" in "z1" receives fewer than 15 messages over 10 seconds
    And BGP neighbor "192.168.90.1" in "z2" receives fewer than 15 messages over 10 seconds
