@serial
@bgp_update_group_source_withdraw
Feature: Withdrawing a route from its source member must not clobber a group-mate's pending advertisement (IPv4)
  ...
  Scenario: Setup topology; both clients hold the prefix via the eBGP neighbor
    ...
    And I create namespace "z1" with IP "192.168.80.1/24" on bridge "br0"
    And I create namespace "z2" with IP "192.168.80.2/24" on bridge "br0"
    And I create namespace "z3" with IP "192.168.80.3/24" on bridge "br0"
    And I create namespace "z4" with IP "192.168.80.4/24" on bridge "br0"
    ...
    Then BGP session in "z1" to "192.168.80.2" should eventually be "Established"
    And BGP session in "z1" to "192.168.80.3" should eventually be "Established"
    And BGP session in "z1" to "192.168.80.4" should eventually be "Established"
    And show command "show bgp" in namespace "z2" should eventually contain "10.80.5.0/24"
    And show command "show bgp" in namespace "z3" should eventually contain "10.80.5.0/24"
    And BGP best path in "z2" for "10.80.5.0/24" has next-hop "192.168.80.4"
    And BGP best path in "z3" for "10.80.5.0/24" has next-hop "192.168.80.4"
  ...
  Scenario: The reflector's best flips to z3's own path; z2 must be advertised the new path
    ...
    Then show command "show bgp 10.80.5.0/24" in namespace "z1" should eventually contain "192.168.80.3"
    And show command "show bgp" in namespace "z2" should eventually contain "192.168.80.3"
    And BGP best path in "z2" for "10.80.5.0/24" has next-hop "192.168.80.3"

  Scenario: The eBGP originator withdraws; z2 keeps the prefix via z3
    Given the test topology exists
    When I apply config "z4-withdrawn.yaml" to namespace "z4"
    Then show command "show bgp 10.80.5.0/24" in namespace "z1" should eventually not contain "192.168.80.4"
    And show command "show bgp" in namespace "z2" should contain "10.80.5.0/24"
    And BGP best path in "z2" for "10.80.5.0/24" has next-hop "192.168.80.3"
