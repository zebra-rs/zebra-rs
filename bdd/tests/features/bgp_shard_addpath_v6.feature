@serial
@bgp_shard_addpath_v6
Feature: BGP IPv6-unicast AddPath session-up sync at N>1 (all paths sync; per-path withdraw + peer-down)
  ...
  Scenario: the late AddPath peer z4 gets BOTH paths on sync
    ...
    And show command "show bgp ipv6 2001:db8:beef::/64" in namespace "z4" should contain "65003 65001"
    And show command "show bgp ipv6 2001:db8:beef::/64" in namespace "z4" should contain "65003 65002"
  ...
  Scenario: z1 withdraws; only z1's path-id is withdrawn from the synced z4
    ...
    And I wait 10 seconds for BGP to operate
    Then show command "show bgp ipv6 2001:db8:beef::/64" in namespace "z4" should not contain "65003 65001"
    And show command "show bgp ipv6 2001:db8:beef::/64" in namespace "z4" should contain "65003 65002"

  Scenario: z2's session drops; the surviving path is withdrawn from z4 too
    Given the test topology exists
    Then show command "show bgp ipv6 2001:db8:beef::/64" in namespace "z4" should contain "65003 65002"
    ...
    And show command "show bgp ipv6 2001:db8:beef::/64" in namespace "z4" should not contain "65003 65002"
