@serial
@bgp_enforce_first_as_local_as
Feature: enforce-first-as judges the AS_PATH the neighbor sent, not our local-as prepend (IPv4)
  ...
  z2 originates 10.46.2.0/24 with a correct first AS.

  h3 runs tests/scripts/bgp_attr_inject_send.py. At session-up it
  announces 10.46.3.0/24 with AS_PATH "65003". On the trigger file
  /tmp/bgp_enforce_first_as_local_as.go it sends, in one write,
  10.46.3.0/24 again with AS_PATH "65099 65003" (a foreign first AS,
  replacing the valid path) and the control prefix 10.46.4.0/24 with
  ...
  - z1.yaml: DUT — enforce-first-as toward z2 (with local-as 64999) and
    h3 (passive).
  - z2.yaml: the not-yet-migrated neighbor; originates 10.46.2.0/24.

  Scenario: Setup topology and establish sessions
    Given a clean test environment
    When I create bridge "br0"
    And I create namespace "z1" with IP "192.168.46.1/24" on bridge "br0"
    And I create namespace "z2" with IP "192.168.46.2/24" on bridge "br0"
    And I create namespace "h3" with IP "192.168.46.3/24" on bridge "br0"
    ...
    Then BGP session in "z1" to "192.168.46.2" should eventually be "Established"
    When I spawn "timeout 600 python3 tests/scripts/bgp_attr_inject_send.py 192.168.46.1 65003 192.168.46.3 4 192.168.46.3 /tmp/bgp_enforce_first_as_local_as 10.46.3.0/24 -- 10.46.3.0/24@65099,65003 10.46.4.0/24" in namespace "h3"
    Then BGP session in "z1" to "192.168.46.3" should eventually be "Established"

  Scenario: The neighbor without local-as passes the check with a correct first AS
    Given the test topology exists
    Then show command "show bgp" in namespace "z1" should eventually contain "10.46.3.0/24"
    And BGP route in "z1" has "10.46.3.0/24" with "as_path" value "65003"
  ...
  Scenario: The local-as ingress prepend does not trip enforce-first-as
    ...
    Then show command "show bgp" in namespace "z1" should eventually contain "10.46.2.0/24"
    And BGP route in "z1" has "10.46.2.0/24" with "as_path" value "64999 65001"

  Scenario: A route that fails the check withdraws the neighbor's earlier path
    Given the test topology exists
    When I execute "touch /tmp/bgp_enforce_first_as_local_as.go" in namespace "h3"
    # The control prefix came in the same write, after the failing UPDATE:
    # once it is in, the failing UPDATE has been processed.
    Then show command "show bgp" in namespace "z1" should eventually contain "10.46.4.0/24"
    # Pre-fix z1 dropped the failing UPDATE and kept the "65003" path h3
    # had just replaced.
    And show command "show bgp" in namespace "z1" should not contain "10.46.3.0/24"
    And BGP session in "z1" to "192.168.46.3" should be "Established"
    And show command "show bgp" in namespace "z1" should contain "10.46.2.0/24"

  Scenario: Teardown topology
    Given the test topology exists
    When I execute "rm -f /tmp/bgp_enforce_first_as_local_as.go" in namespace "h3"
