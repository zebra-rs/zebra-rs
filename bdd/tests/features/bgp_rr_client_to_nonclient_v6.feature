@serial
@bgp_rr_client_to_nonclient_v6
Feature: A route reflector reflects a client's route to non-client iBGP peers (IPv6)
  ...
  Scenario: Setup topology and establish all four sessions
    ...
    And I create namespace "z1" with IP "2001:db8:50::1/64" on bridge "br0"
    And I create namespace "z2" with IP "2001:db8:50::2/64" on bridge "br0"
    And I create namespace "z3" with IP "2001:db8:50::3/64" on bridge "br0"
    And I create namespace "z4" with IP "2001:db8:50::4/64" on bridge "br0"
    And I create namespace "z5" with IP "2001:db8:50::5/64" on bridge "br0"
    ...
    Then BGP session in "z1" to "2001:db8:50::2" should eventually be "Established"
    And BGP session in "z1" to "2001:db8:50::3" should eventually be "Established"
    And BGP session in "z1" to "2001:db8:50::4" should eventually be "Established"
    And BGP session in "z1" to "2001:db8:50::5" should eventually be "Established"

  Scenario: Client to client — the other client receives the reflected route (control)
    Given the test topology exists
    Then show command "show bgp ipv6" in namespace "z3" should eventually contain "2001:db8:20::/64"
    And show command "show bgp ipv6 2001:db8:20::/64" in namespace "z3" should eventually contain "Originator: 10.50.0.2"
    And show command "show bgp ipv6 2001:db8:20::/64" in namespace "z3" should contain "Cluster list:"
  ...
  Scenario: Non-client to client — the clients receive the reflected route (control)
    ...
    And show command "show bgp ipv6" in namespace "z3" should eventually contain "2001:db8:40::/64"
    And show command "show bgp ipv6 2001:db8:40::/64" in namespace "z3" should eventually contain "Originator: 10.50.0.4"
    And show command "show bgp ipv6 2001:db8:40::/64" in namespace "z3" should contain "Cluster list:"
  ...
  Scenario: Client to non-client — every non-client iBGP peer receives the client's route
    ...
    And show command "show bgp ipv6" in namespace "z5" should eventually contain "2001:db8:20::/64"
    And show command "show bgp ipv6 2001:db8:20::/64" in namespace "z4" should eventually contain "Originator: 10.50.0.2"
    And show command "show bgp ipv6 2001:db8:20::/64" in namespace "z4" should contain "Cluster list:"
    And show command "show bgp ipv6 2001:db8:20::/64" in namespace "z5" should contain "Originator: 10.50.0.2"
  ...
  Scenario: Non-client to non-client — a non-client's route is NOT reflected to another non-client (control)
    ...
    And show command "show bgp ipv6" in namespace "z5" should not contain "2001:db8:40::/64"
  ...
  Scenario: Withdrawing the client's route removes it from the non-clients too
    ...
    Then show command "show bgp ipv6" in namespace "z4" should eventually not contain "2001:db8:20::/64"
    And show command "show bgp ipv6" in namespace "z5" should eventually not contain "2001:db8:20::/64"
    And show command "show bgp ipv6" in namespace "z3" should eventually not contain "2001:db8:20::/64"
