...
`unicast_next_hop_self` / `unicast_next_hop_unchanged`,
`vpnv4_next_hop_self` / `vpnv4_next_hop_unchanged` and
`vpnv6_next_hop_self` / `vpnv6_next_hop_unchanged` — see the signature
table above.)
...
| **Transient bad grouping during config commit** as multiple leaves fire, or a signature field changed on an Established peer with no regroup at all (the signature used to be computed only on the Established edge). | Medium | Implemented: the outbound policy / prefix-set binding handlers re-form the peer's groups synchronously (`update_group::regroup_if_stale`, before the policy actor's reply runs the soft-out), and `CommitEnd` runs a stale-signature sweep over every Established peer (`regroup_stale_peers`) followed by an outbound re-sync of each mover, so every other signature knob takes effect within its commit. A peer is moved family by family. While its old group has a flush job in flight it is frozen instead: it keeps the membership (its deferred withdraws follow the job) but is skipped by every fan-out and by any new job, so its changed settings never enter the shared memo or cache; `flush_done_*` then moves it and the caller re-syncs it (IPv4 and, since this change, IPv6). |
