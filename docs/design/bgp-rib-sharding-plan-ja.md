...
（BatchAfi/LabeledAfi）。YANG ノブ（`shards`、`peer-task`）。2 つのオプトイン
なオフメイン egress モデル（ピアごと + update-group ごとのタスク）。N>1 の
読み取りパス（show / summary / sync）と N>1 の `network` 起点化（origination）
が完了** — Phase 0 + A は 2026-06-12 にマージ
（PR #1402/#1406/#1408/#1416）。その後、シャーディングブランチ全体が
**`main` に 2026-06-16 マージ済み（PR #1442）**。以下はすべて WIP ブランチでは
なく `main` 上にある。それ以降のフォローオン作業 — YANG ノブ、2 つのオプトイン
な egress タスクモデル（ピアごと + update-group ごと）、N>1 の読み取りパス
（show / summary / sync）、N>1 の `network` 起点化 — は PR #1444–#1458 で
マージ済み。§5–8 の計画から意図的に逸脱した
3 点: **B.3 はタスクの spawn ではなく同期ディスパッチになった**、
**再スコープした Phase C は純粋なポリシーウォークを並列化（rayon）する**、
**マルチシャードのファンアウトは tokio タスクではなく専用 OS スレッド上で動く**。
シャード数は **コンフィグまたは環境変数で駆動** される — YANG ノブ
`router bgp shards <1-64>`（PR #1444）または `ZEBRA_BGP_SHARDS`（1–64 に
クランプ、デフォルト 1）で、egress プールのサイズは
`ZEBRA_BGP_UPDATE_WORKERS`（デフォルト `max(1, cores − shards)`）で決まる。
コンパイル時の `SHARDS` 定数は廃止された。egress モデルも同じ方法で選択する:
デフォルト（update-group フラッシュ）、`router bgp peer-task true` /
`ZEBRA_BGP_PEER_TASK`（ピアごとのタスク、GoBGP モデル）、または
`ZEBRA_BGP_EGRESS_GROUP_TASK`（update-group ごとのタスク、Juniper の
update-worker 形態）— 後述の「egress モデル」を参照。
...
トレイトの背後に機能する Adj-RIB-Out を持つようになった。そしてこの土台の
上に、2 つの **オプトインなオフメイン egress モデル** が構築された: ピアごとの
タスク（PET、GoBGP 形態）と update-group ごとのタスク（Juniper の update-worker
形態、§12 P1 の最初の実装）。後述の「egress モデル」を参照。§5–10 は元々の設計
...
### Egress モデル — 3 つの選択可能な形態（構築済み、2 つはオプトイン）

advertise パスは現在 3 つの形態を持ち、起動時に選択される（egress モデルは
インスタンスの生存期間中固定、一度だけ読まれる）。3 つとも上記の統一された
Adj-RIB-Out の上に乗っており、違いは **ピアごとの adj-out + encode がどこに
あるか** である。

- **デフォルト — update-group flush（メインタスク）。** reduce が
  `BestPathV4` デルタを `Arc<BgpAttr>` をキーとしたグループごとのキャッシュに
  バケット化し、デバウンスし、バケットごとに 1 つの UPDATE を
  `spawn_blocking` でエンコードしてメンバーに複製する（後述の「Egress flush」
  図）。encode はオフスレッド。バケット化 / キャッシュ / adj-out はメイン上。
  デフォルトの全実行と、egress タスク機能以外の全 BDD はこれを使う。
- **ピアごとの egress タスク（PET）— `router bgp peer-task true` /
  `ZEBRA_BGP_PEER_TASK`**（`bgp/peer_egress.rs`）。GoBGP のピアごとモデル:
  Established な各ピアにつき 1 タスクが、そのピアの v4 `adj_out` + 専用の attr
  インターナ + `SyncCtx` を所有し、build + out-policy + encode + 送信を自分で
  行う — **update-group のコアレッシングなし**（各ピアが自分のバイト列を
  エンコードする）。reduce の advertise シンク（`apply_ipv4_advertise_job`）と
  起点化 / 直接 withdraw のシンク（`route_advertise_to_peers`）の両方が
  ゲート on で `fan_advertise_to_pets` に分岐するので、ピアごとの egress は
  メインタスクの外で動く。v4-unicast のみ、デフォルト off。
  `@bgp_peer_egress_v4` でロック。
- **update-group ごとの egress タスク — `ZEBRA_BGP_EGRESS_GROUP_TASK`**
  （`bgp/group_egress.rs`）。Juniper の update-worker 形態であり、§12 P1 の
  最初の実装: **`UpdateGroup` ごとに 1 つ、M タスク**（ピアごとの N ではない）。
  各タスクがグループ共有の `adj_out` を所有し、各 best パスを **一度だけ**
  エンコードし、そのバイト列をメンバーピアに **ファンアウト** する（パスの
  ソースは除外 — fan 時のスプリットホライズン）。PET は M=1 の縮退ケース。
  phase 0–5 + AddPath（PR #1453/#1454/#1456）で構築: ライフサイクル
  （attach/detach がタスクを spawn/retire）、イベント駆動の advertise/withdraw
  （`fan_advertise_to_groups` + `fan_addpath_to_groups` — AddPath ピアは全候補を
  受け取る）、peer-down、遅延ピアのセッション確立同期（`RecordAdjOut`）、
  soft-out の再 fan、show/summary の読み取り（後述）。デフォルト off、ゲート
  off ではバイト単位で同一。`@bgp_egress_group_task`（N=1）+
  `@bgp_egress_group_sharded`（N>1）+ `@bgp_addpath_group` でロック。

**デフォルトの切り替え**（いずれかのタスクモデルを出荷時の egress にする）は
まだ将来 — ゲートの優先順位（明示的な env と group デフォルト）、新デフォルトでの
完全な BGP BDD 再検証、group タスクが PET を包含した後の PET 廃止が必要。
タスクモデルでの **v6/VPN egress** もまだ将来（今は v4-unicast のみ）。
...
- **egress タスクのデフォルト切り替え + v6/VPN egress** — 上記「Egress
  モデル」参照: PET / group タスクモデルは v4-unicast のみ・デフォルト off。
  いずれかを出荷時の egress にすることと v6/VPN への拡張は、どちらも将来。

**今サイクルでクローズ（かつての「直近のギャップ」）。**

- **N>1 の v4-unicast read パス（B.4）— 完了。** `show bgp ipv4`、
  セッション確立時の `route_sync_ipv4`、`clear`/soft-in、*さらに* `show bgp
  [ipv4] summary` が、いまや権威あるオフメインの所有者から読むようになった —
  read レプリカミラー + `DumpV4` シャード同期 + show の scatter-gather
  （received-routes、advertised-routes、summary カウント、PR #1455/#1457）。
  以下の（更新された）B.4 セクションを参照。
- **YANG ノブ — 完了。** `router bgp shards <1-64>`（PR #1444）が
  `ZEBRA_BGP_SHARDS` を出荷形態として置き換える。`router bgp peer-task
  <bool>`（PR #1450）が egress モデルを選択する。C.4 の残り: 性能行列 + 根拠の
  ある出荷デフォルト。
- **N>1 の `network` 起点化 — 完了（PR #1458）。** `route_add` /
  `route_del` は起点化された行を main シャードに直接書いていたが、プールの
  セッション確立時 `DumpV4` はそこを見ないので、起点化後に確立したピアは
  ネットワークを学習しなかった。プール経由で注入する
  （`ShardMsg::OriginateV4`）ことで、ピアルートと全く同じくプール所有に修正。
  **N>1 での他の main 側 v4 注入（redistribute、aggregate、default-originate）
  も同じ「プール経由で注入する」パターンに従う必要があり**、`bgp.shard.v4` を
  直接書いてはならない。
...
ピアごとの **show** の読み取りも N>1 で着地した。いずれも
`inst.rs::process_show_msg`（async）で同期 `show_cb` の手前でインターセプトし、
オフメインの所有者から oneshot で scatter-gather する — 同期レンダラは空の
main 側コピーを読んで空/0 を表示してしまうため:

- **`show bgp neighbors <peer> received-routes`** — Adj-RIB-In は N>1 で
  プールシャードに住む。`ShardMsg::DumpAdjInV4` で収集（A2 ⑤）。
- **`show bgp neighbors <peer> advertised-routes`** — Adj-RIB-Out はゲート on で
  PET / group タスクに住む。`request_adj_out` で要求。
- **`show bgp [ipv4] summary`** の PfxRcd / PfxSnt — PfxRcd はシャードから、
  PfxSnt は PET または group タスクから。フルダンプではなく **カウントのみ** の
  メッセージ（`CountAdjInV4All`、`EgressDeltaV4::CountAdjOut`、
  `GroupEgressDeltaV4::CountAdjOut`）を使う（PR #1455/#1457）。

原則: ゲート on / N>1 で Adj-RIB を読む新しいピアごとの show は、同じく
async で gather しなければならない。さもないと黙って空を表示する。残作業:
ストリーム化 / ページング化した `show`（`show` RPC の 4 MB ソート済みテーブル
上限）と、`clear`/soft-in を同じ `DumpV4` に載せ替えること。
...
| RTO をパックする update スレッド | `UpdateGroup` キャッシュ + debounce タイマ + 正規エンコードを所有する update-worker タスク | 🔶 部分的 — エンコードはスレッド外（A.2 `FlushJob` → `spawn_blocking`）で、out-policy 事前計算は並列化（E.1/E.2 上限付き egress プール）。専用グループ親和性ワーカーは **オプトインの egress-group-task として構築済み**（#1453/#1454、デフォルト off、シャードごとジャーナルではなく main reduce 経由で供給）— 「Egress モデル」+ §12 P1 を参照 |
...
シャーディングはデフォルト off で出荷される（`router bgp shards` /
`ZEBRA_BGP_SHARDS` 未設定 → N=1）。一連の作業は現在すべて **`main` にマージ
済み**（`bgp-nshard-policy-shard` ブランチは PR #1442、2026-06-16 に着地。
YANG ノブと egress タスクモデルはその後 #1444–#1458 で着地）。ステータス列は
2026-06-16 時点の `main` を反映する。
...
| B.4 | Show / clear / sync scatter-gather | B.3 | ✅ 構築済み — read レプリカミラー + `DumpV4` セッション確立時同期 + ピアごとの show scatter-gather（`show bgp ipv4`、received-/advertised-routes、`summary` カウント、PR #1442/#1455/#1457）。`@bgp_shard_v4_sync` + フル同期行列グリーン。残り: `clear`/soft-in を `DumpV4` に、ストリーム化 `show` |
| B.5 | N=1 での BDD + ライフサイクル硬化 | B.4 | ⏳ |
| C.1 | prefix ハッシュで N シャードへファンアウト（+ YANG ノブ） | B.5 | ✅ 構築済み — 専用スレッド `ShardPool`、`router bgp shards <1-64>`（#1444）または `ZEBRA_BGP_SHARDS`（プレーン v4-unicast のみ。VPNv4/v6/VPNv6/LU は依然同期） |
| C.2 | update-worker タスク（グループ親和性） | A.2, C.1 | 🔶 部分的 — E.1/E.2 並列 egress（上限付きプール）構築済み。専用グループ親和性ワーカーは **オプトインの egress-group-task** として構築済み（#1453/#1454、デフォルト off）— §12 P1 の最初の実装。デフォルト切り替え + v6/VPN は将来 |
| C.3 | バリア: EoR、refresh、GR/LLGR スイープ、clear | C.1 | ⏳ |
| C.4 | 性能行列、デフォルト、ドキュメント | C.2, C.3 | 🔶 部分的 — YANG ノブ着地（`shards` #1444、`peer-task` #1450）。性能行列 + 根拠ある出荷デフォルトは TODO |
...
**最初の実装は構築済み（オプトイン、`ZEBRA_BGP_EGRESS_GROUP_TASK`）。**
**egress-group-task**（`bgp/group_egress.rs`、上記「Egress モデル」）がこの
ワーカーである — ジャーナルを除いて: 静的な group→task 親和性を持つ M タスクが、
各々自グループの `adj_out` + encode を所有し、バイト列をメンバーにファンアウト
する（fan 時のスプリットホライズン）。update-group のライフサイクルが
spawn/retire する。シャードはまだ *main reduce 経由*（`fan_advertise_to_groups`）
で供給し、ワーカーが pull するシャードごとの `lfjour` ではない — つまり reduce
ホップと上限なしのファンアウト（P2）は残る。完全な P1 への残作業: シャードごと
ジャーナル（ワーカーが pull、main はファンアウトしない）、続いていずれかの
タスクモデルをデフォルト egress にして（今は v4-unicast のみ + デフォルト off）
PET を廃止すること。ピアごと（PET）と group ごとのタスクは、オフメイン egress
の機構（ライフサイクル、同期、soft-out、show/summary 読み取り、AddPath）を既に
エンドツーエンドで検証している。
