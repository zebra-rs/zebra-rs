...
- **Phase 8 (SR-MPLS variant) — IN PROGRESS, sliced.** Carries Risk #2 (Linux has no per-context label table; the dataplane uses a global-ILM approximation valid for structured dual-homing/lab, documented).
  ...
  - **Next:** slice 2 (`IlmType::ContextLabel` + `ilm_add` — pop the context label, deliver into the protector's VRF); slice 3 (PLR push `backup.mpls_label = [repair…, context_label]`); slice 4 (SR-MPLS L3VPN BDD + dual-homing failover).
