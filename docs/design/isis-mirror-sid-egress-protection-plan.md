...
- **Next:** Phase 7 — end-to-end **BDD** (node protection: kill PEA, VPN ping recovers via PEB End.M; then PE–CE link protection). Validates the two-level seg6local decap and the protect_switch reroute on real namespaces. Then Phase 8 (SR-MPLS), Phase 9 (hardening).
