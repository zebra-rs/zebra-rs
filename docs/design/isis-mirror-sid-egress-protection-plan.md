...
- **Phase 9 hardening** — `show` surfacing of the redirected (B6/H.Encaps) form (today `show segment-routing srv6 sid` keeps showing the canonical End.DT46; the kernel route reflects the live redirect); ingress **BGP-PIC** for an end-to-end node-down *service* failover; and SR-MPLS node protection (blocked on stock Linux by Risk #2 — no per-context label table; needs eBPF/VPP).
