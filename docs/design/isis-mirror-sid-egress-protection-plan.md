...
- **Phase 9 hardening** — a proper egress-protection *withdrawal* path (clear `egress_protect` when the protector's LSP is present but drops the Mirror SID, rather than only sticky-keep); `show` surfacing of the redirected (B6/H.Encaps) form (today `show segment-routing srv6 sid` keeps showing the canonical End.DT46; the kernel route reflects the live redirect); a time-bounded hold-down on the retained backup (today it floats for as long as the protector advertises); and SR-MPLS node protection (blocked on stock Linux by Risk #2 — no per-context label table; needs eBPF/VPP).
