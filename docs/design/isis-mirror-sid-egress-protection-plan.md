...
- **Next:** Phase 7b — live-traffic + via-vrf BDD (BGP L3VPN service, dual-homed CE, kill PEA, VPN ping recovers via PEB End.M) — validates the two-level decap + `protect_switch` failover with real traffic. Then Phase 8 (SR-MPLS), Phase 9 (hardening, incl. PE–CE link protection).
