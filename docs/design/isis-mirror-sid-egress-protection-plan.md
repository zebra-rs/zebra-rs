...
- **Phase 8 (SR-MPLS variant) — IN PROGRESS, sliced.** Carries Risk #2 (Linux has no per-context label table; the dataplane uses a global-ILM approximation valid for structured dual-homing/lab, documented).
  ...
  - **Next:** slice 3 (egress redirect — the protected egress, on PE-CE link down, swaps its own VPN-label ILM to push `[transport-to-protector, context_label]`, the MPLS analog of the SRv6 End.B6.Encaps redirect); slice 4 (SR-MPLS L3VPN BDD + dual-homing failover — builds the live SR-MPLS topology and validates the whole chain).
