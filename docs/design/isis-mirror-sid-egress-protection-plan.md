...
- **Phase 1a — SRv6 codec — DONE** (branch `isis-mirror-sid-codec`, off `origin/main`). `crates/isis-packet/`:
  ...
  - Green: `cargo test -p isis-packet` (52 tests), `cargo fmt`, `cargo clippy --workspace --all-targets -- -D warnings`. Not yet committed/pushed (awaiting user).
- **Next:** Phase 1b (SR-MPLS Binding TLV 149 + M-flag codec) — or skip straight to Phase 2 (config) since SRv6 lands first and SR-MPLS is Phase 8. Recommend Phase 2 next.
