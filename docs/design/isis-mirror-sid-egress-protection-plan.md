...
- **Next:** Phase 7b proper — now that the VPN service forwards, build the live-traffic + via-vrf failover BDD on top (dual-homed CE, kill PEA, VPN ping recovers via PEB End.M) — validates the two-level decap + `protect_switch` failover with real traffic. Then Phase 8 (SR-MPLS), Phase 9 (hardening, incl. PE–CE link protection).
