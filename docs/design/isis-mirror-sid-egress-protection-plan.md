...
- **Next:** Phase 6b — the PLR **repair install**: for routes egressing via a protected node, build a `Nexthop::Protect` backup that pushes the Mirror SID (+ TI-LFA repair list to the protector) and wire the BFD `protect_switch` trigger. Then Phase 7 (end-to-end BDD).
