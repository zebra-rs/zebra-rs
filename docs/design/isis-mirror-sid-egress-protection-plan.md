...
- **SRv6 node-protection stale-route retention (Risk #4) — DONE** (branch `isis-mirror-sid-node-protection`, off merged `main`; **5/5 BDD scenarios green on real namespaces**). Closes the durability gap in the Phase 6b PLR repair: that repair is a `Nexthop::Protect` backup on the route to the protected egress's locator, so it covers only the sub-second BFD `protect_switch` window — once the egress's *node* fails and its LSP ages out, the locator leaves the SPF and the diff withdraws the whole route (repair included).
  - **Mechanism (`reconcile_retained_locators`, run each SPF in `apply_routing_updates`).** For every received Mirror SID whose protector is reachable, pre-install a **floating backup**: a seg6 H.Encaps route to the Mirror SID at administrative distance **250** (`RibType::Static`, a *distinct* rtype so it does not collide with the native IS-IS route under the RIB's per-rtype replacement — which would orphan one in the kernel). The native locator route (distance 115) wins while the egress is up, so the backup sits in the RIB unselected; when the node fails and the native route is withdrawn, the RIB's best-path promotes the backup, carrying locator-bound traffic to the protector (PEB's End.M) and surviving SPF reconvergence; when the egress returns, best-path demotes it again. Reconciled against `Isis::retained_locators` (per level); withdrawn when the protector/protection goes away.
  - **Guard:** skip the node's **own** locator (`top.sr_locator…prefix`) — a remote protector's offer for our own locator is not ours to retain (else the protected egress installs a distance-250 backup for its own locator toward the protector; harmless only because the local uN route wins, but wrong).
  - **Why not a service ping in the BDD:** a node-down *service* failover is masked — when pea dies its BGP withdraws, the ingress loses the route (or, if PEB re-advertises, reconverges to PEB's own path, bypassing the Mirror SID). The seg6 H.Encaps **forwarding** is already traffic-validated by `@mirror_sid_egress_link`; `@mirror_sid_node_base` (pe1 PLR + stub pea + pe1↔peb bypass, fast hellos) deterministically validates the **control/FIB plane**: native via `pe1-pea` in steady state → stop pea → the locator route survives as a seg6 redirect to peb's Mirror SID → restart pea → native again.
- **Node-protection hold-down — DONE** (branch `isis-mirror-sid-holddown`, off merged `main`; **2 scenarios added to `@mirror_sid_node_base`, 7/7 green**). `router isis egress-protection hold-down <seconds>` bounds how long the retained backup forwards after the egress's node fails, then withdraws it so a genuinely-decommissioned egress is not masked toward the protector forever. `RetainEntry { installed, holddown: Option<Timer> }` per locator: the reconcile arms a one-shot `Timer::once` (firing `Message::EgressRetentionExpire`) when the egress goes down and the backup is live; `egress_retention_expire` withdraws the backup if the egress is still down on expiry, and the reconcile re-installs it if the egress later returns. `0`/unset keeps the historical float-forever behavior (so the SRv6 link-protection and control-plane suites are unaffected — still 6/6 and 5/5).
- **Egress-protection withdrawal path — DONE** (branch `isis-mirror-sid-withdrawal`, off merged `main`). The RIB's `egress_protect` registration was kept **sticky** (an empty `EgressProtectSet` scan never dropped the last-known protector) so a convergence-transient empty LSDB read could not disarm protection mid-failure — but that also meant a protector that genuinely withdrew its Mirror SID (LSP present, no longer advertising) was masked forever. The sticky/withdrawal decision moves into IS-IS `register_egress_protections`, which has the LSDB: it now tracks the prior registration with its protector (`Isis::egress_protect_registered`), and carries a no-longer-advertised protection forward **only when its protector's LSP is absent** (transient) — dropping it when the protector is present but silent (genuine withdrawal). The RIB handler becomes an authoritative replace (sticky logic removed). Pure `authoritative_protections` unit test covers all three cases; `@mirror_sid_srv6` gains a live scenario (peb deletes its Mirror SID → the PLR's backup and received view clear). Link-protection 6/6 and node 7/7 unaffected.
- **`show` surfacing of the live redirect — DONE** (branch `isis-mirror-sid-show-redirect`, off merged `main`). `show segment-routing srv6 sid` previously reported only a redirected SID's canonical End.DT46 behavior while the kernel route held the live `seg6 H.Encaps` to the protector's Mirror SID. `Rib::redirected_sids` becomes a map `sid_addr → mirror_sid` (it already latched the live FIB form; now it carries the target), so `sid_show` annotates each redirected SID with `-> egress-protection redirect: H.Encaps to <mirror_sid>` (and a `redirected_to` JSON field) **independently of the volatile `egress_protect` registration**. `@mirror_sid_egress_link` asserts the annotation appears on PE-CE link down and clears on recovery.
  - **Regression fixed along the way:** the withdrawal PR's authoritative-replace made `egress_protect` vulnerable to the "1 LSP transient" — a PE-CE link-down momentarily collapses the SPF's LSDB view to the local node only (`live_nodes == 1`), and the empty scan cleared the registration (the old sticky guarded exactly this). `register_egress_protections` now skips the update on a degenerate (`live_nodes <= 1`) LSDB; genuine withdrawal still runs against a healthy multi-node LSDB (the `@mirror_sid_srv6` withdrawal scenario stays green).
- **Ingress BGP-PIC — DONE** (branch `bgp-pic-nht-retention`, off merged `main`; **`@mirror_sid_node_vpn` 4/4 green**). Closes the node-down *service* failover the `@mirror_sid_node_base` control-plane test deferred: a real BGP L3VPN ping survives PEA's node death end to end. Three composing pieces, all opt-in/no-regression (`@mirror_sid_egress_link` 6/6 and `@mirror_sid_vpnsrv6_base` 4/4 unaffected):
  - **`neighbor X pic-retention`** (per-neighbor presence container, default off) — on a session drop, `route_clean` marks the peer's VPN routes **stale** and keeps them (reusing the LLGR stale path, minus the NO_LLGR/`LLGR_STALE` semantics) instead of withdrawing, with a safety `PIC_RETENTION_HOLDDOWN` (600 s) stale timer. NHT is the primary withdrawal trigger.
  - **NHT tracks the End.DT46 SID, not PEA's loopback** for SRv6 L3VPN (`nht::nht_target`, swapped in at the register / transport / re-eval / untrack sites). PEA's loopback ages out of the IGP on node death, but its *locator* is held by the node-protection stale-route retention — so tracking the SID keeps the route reachable. The fix had to extend to the shard's `candidate_nexthops_*` (survivor set fed to `nht_untrack_withdrawn`), or the re-import untracked the SID and the transport went empty.
  - **Single-segment H.Encaps + kernel re-lookup** — the ingress installs `encap seg6 [PEA-SID]` (segleft 0); the kernel re-routes that packet by its new DA, the retained locator route H.Encaps a *second* single-segment `[Mirror SID]`, and PEB's End.DT6 (segleft 0) decaps in its mirror context. Stacking both SIDs in one SRH was wrong — it leaves segleft=1 at PEB. This is exactly the egress-link redirect shape, now initiated at the ingress PLR.
- **Phase 9 remaining** — SR-MPLS node protection (blocked on stock Linux by Risk #2 — no per-context label table; needs eBPF/VPP).
