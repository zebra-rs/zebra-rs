...
install:
	...
	# resolves to /usr/share/zebra-rs/yang, so a stale copy here silently
	...
	sudo mkdir -p /usr/share/zebra-rs/yang
	sudo cp zebra-rs/yang/*.yang /usr/share/zebra-rs/yang/
