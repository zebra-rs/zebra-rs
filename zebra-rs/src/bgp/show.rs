...
fn show_bgp_attributes(
    ...
    let mut buf = String::new();
    let mut seen: HashSet<*const BgpAttr> = HashSet::new();
    let mut attrs: Vec<Arc<BgpAttr>> = Vec::new();

    // Collect unique Arc<BgpAttr> from local_rib v4 candidates.
    for (_prefix, ribs) in bgp.local_rib.v4.0.iter() {
        for rib in ribs.iter() {
            let ptr = Arc::as_ptr(&rib.attr);
            if seen.insert(ptr) {
                attrs.push(Arc::clone(&rib.attr));
            }
        }
    }
    // Local_rib v4 selected.
    for (_prefix, rib) in bgp.local_rib.v4.1.iter() {
        let ptr = Arc::as_ptr(&rib.attr);
        if seen.insert(ptr) {
            attrs.push(Arc::clone(&rib.attr));
        }
    ...
    // Local_rib v4vpn.
    for (_rd, table) in bgp.local_rib.v4vpn.iter() {
        for (_prefix, ribs) in table.0.iter() {
            for rib in ribs.iter() {
                let ptr = Arc::as_ptr(&rib.attr);
                if seen.insert(ptr) {
                    attrs.push(Arc::clone(&rib.attr));
        ...
        for (_prefix, rib) in table.1.iter() {
            let ptr = Arc::as_ptr(&rib.attr);
            if seen.insert(ptr) {
                attrs.push(Arc::clone(&rib.attr));
            }
    ...
    // Peers adj_in and adj_out.
    for (_addr, peer) in bgp.peers.iter() {
        for (_prefix, ribs) in peer.adj_in.v4.0.iter() {
            for rib in ribs.iter() {
                let ptr = Arc::as_ptr(&rib.attr);
                if seen.insert(ptr) {
                    attrs.push(Arc::clone(&rib.attr));
        ...
        for (_rd, table) in peer.adj_in.v4vpn.iter() {
            for (_prefix, ribs) in table.0.iter() {
                for rib in ribs.iter() {
                    let ptr = Arc::as_ptr(&rib.attr);
                    if seen.insert(ptr) {
        ...
        for (_prefix, ribs) in peer.adj_out.v4.0.iter() {
            for rib in ribs.iter() {
                let ptr = Arc::as_ptr(&rib.attr);
                if seen.insert(ptr) {
                    attrs.push(Arc::clone(&rib.attr));
        ...
        for (_rd, table) in peer.adj_out.v4vpn.iter() {
            for (_prefix, ribs) in table.0.iter() {
                for rib in ribs.iter() {
                    let ptr = Arc::as_ptr(&rib.attr);
                    if seen.insert(ptr) {
    ...
    writeln!(buf, "BGP Attributes (unique: {})", attrs.len())?;
    writeln!(buf)?;

    for (i, attr) in attrs.iter().enumerate() {
        // Subtract 1 from strong_count to exclude our temporary clone in attrs vec.
        let refcount = Arc::strong_count(attr) - 1;
        writeln!(buf, "Attribute[{}] refcount {}", i + 1, refcount)?;

        writeln!(buf, "  Nexthop: {}", show_nexthop(attr))?;
        if let Some(ref origin) = attr.origin {
            let origin_str = match origin {
                Origin::Igp => "IGP",
                Origin::Egp => "EGP",
                Origin::Incomplete => "incomplete",
            };
            writeln!(buf, "  Origin: {}", origin_str)?;
        }
        if let Some(ref aspath) = attr.aspath {
            writeln!(buf, "  AS Path: {}", aspath)?;
        }
        if let Some(ref local_pref) = attr.local_pref {
            writeln!(buf, "  Local Pref: {}", local_pref.local_pref)?;
        }
        if let Some(ref med) = attr.med {
            writeln!(buf, "  MED: {}", med.med)?;
        }
        if let Some(ref com) = attr.com {
            writeln!(buf, "  Community: {}", com)?;
        }
        if let Some(ref ecom) = attr.ecom {
            writeln!(buf, "  Ext Community: {}", ecom)?;
        }
        if let Some(ref lcom) = attr.lcom {
            writeln!(buf, "  Large Community: {}", lcom)?;
        }
        if let Some(ref originator_id) = attr.originator_id {
            writeln!(buf, "  Originator ID: {}", originator_id.id)?;
        }
        if let Some(ref cluster_list) = attr.cluster_list {
            let ids: Vec<String> = cluster_list.list.iter().map(|id| id.to_string()).collect();
            writeln!(buf, "  Cluster List: {}", ids.join(" "))?;
        }
        writeln!(buf)?;
    }
