...
| D22 | **Initial admin gates: Apply and Clear RPCs only.** `SessionTable::require_admin` checks `enabled`, enforces sliding TTL + hard cap (with auto-downgrade on expiry), and slides the idle deadline on each authorized call. `DoExec` is intentionally not gated (it carries both completion and command-execution traffic, including the `configure` mode-entry and `commit` commands). Gating mode-entry and `commit` is the natural pair to Phase 5 configure-mode lock and is deferred until that lands. |
