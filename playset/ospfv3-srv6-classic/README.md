...
default route. (The static-route walkthrough below adds some — and
shows why a *plain* recursive static could not have done this job.)
...
book; the [IS-IS classic lab](../isis-srv6-classic/README.md) has the
same walkthrough over an IS-IS underlay.
...
s>show ipv6 route
...
B     2001:db8:200::/64 [200/0] via seg6 [fcbb:bbbb:8:40::], s-n1, 00:00:23
S  *> 2001:db8:200::/64 [1/0] via 2001:db8::8 (recursive), 00:00:03
                              via fe80::29:4bff:fec0:97fa, s-n1
```

The static (distance 1) displaces the BGP service route (200) and
resolves recursively just fine — the covering route is the OSPFv3 one
(`O  *> 2001:db8::8/128 [110/2]`). But that is a *plain* IPv6 route
(SRv6 encapsulates only where a SID says so), so there is no transport
to inherit. The kernel gets a bare `via fe80::… proto static`, the
packet leaves `s` unencapsulated, and it dies one hop in — the core
routes only links, loopbacks, and locators:
...
2001:db8:200::/64 nhid 15  encap seg6 mode encap segs 1 [ fcbb:bbbb:8:e064:: ] via fcbb:bbbb:8:e064:: dev s-n1 proto static metric 1024 onlink pref medium
```
...
13:26:38.464711 IP6 2001:db8:1::1 > fcbb:bbbb:8:e064::: RT6 (len=2, type=4, segleft=0, last-entry=0, tag=0, [0]fcbb:bbbb:8:e064::) IP6 2001:db8:100::100 > 2001:db8:200::100: ICMP6, echo request, id 8043, seq 1, length 64
13:26:38.464746 IP6 2001:db8:10::2 > fcbb:bbbb:1:40::: RT6 (len=2, type=4, segleft=0, last-entry=0, tag=0, [0]fcbb:bbbb:1:40::) IP6 2001:db8:200::100 > 2001:db8:100::100: ICMP6, echo reply, id 8043, seq 1, length 64
```
...
3001:db8::2 nhid 14  encap seg6 mode encap segs 1 [ fcbb:bbbb:8:40:: ] via fcbb:bbbb:8:: dev s-n1 proto static metric 1024 onlink pref medium

s>ip -6 route show 2001:db8:200::/64
2001:db8:200::/64 nhid 14  encap seg6 mode encap segs 1 [ fcbb:bbbb:8:40:: ] via fcbb:bbbb:8:: dev s-n1 proto bgp metric 1024 onlink pref medium
```

Note the two routes share `nhid 14`: the inherited entry is
